  ...
  return <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? "glass-nav py-3" : "bg-transparent py-6"}`}>
      <div className="container mx-auto px-4 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center space-x-8">
          ...
          <a href="#promise" className="text-sm font-medium text-foreground hover:text-accent transition-colors">
            Our Promise
          </a>
          <a href="#the37blog" className="text-sm font-medium text-foreground hover:text-accent transition-colors">
            the37blog
          </a>
      ...
      {isMobileMenuOpen && <div className="md:hidden bg-background/95 backdrop-blur-md border-t border-accent/10 animate-slide-down">
          <div className="container mx-auto px-4 py-4 flex flex-col space-y-4">
            ...
            <a href="#promise" className="py-2 text-foreground hover:text-accent transition-colors" onClick={() => setIsMobileMenuOpen(false)}>
              Our Promise
            </a>
            <a href="#the37blog" className="py-2 text-foreground hover:text-accent transition-colors" onClick={() => setIsMobileMenuOpen(false)}>
              the37blog
            </a>
            <a href="#contact" className="accent-btn inline-block text-center" onClick={() => setIsMobileMenuOpen(false)}>
              Get Started
            </a>
          </div>
        </div>}
